...
public sealed class DocumentDbSet : DbSet<Document>
{
    ...
    public long Count()
    {
        using SqliteCommand command = CreateCommand("SELECT count(*) FROM Document");
    ...
    {
        using SqliteCommand command = CreateCommand("SELECT Id, Label, Tag FROM DocumentType ORDER BY Id");
        using SqliteDataReader reader = command.ExecuteReader();
    ...
    public override bool Contains(Document entity)
    ...
    public override bool AddOrUpdate(Document entity)
        => throw new NotImplementedException();

    public override int Remove(Document entity)
    {
        using SqliteCommand command = CreateCommand("DELETE FROM Document WHERE Id = $id");
        command.Parameters.Add("$id", SqliteType.Integer).Value = entity.Id;
        return command.ExecuteNonQuery();
    }
